...
func main() {
	...
	tracer, err := opzipkin.NewTracer(reporter)
	serverTrace := zipkin.GRPCServerTrace(tracer)

	// 服务器
	bookInfoHandler := kit_grpc.NewServer(makeGetBookInfoEndpoint(), decodeRequest, encodeResponse, serverTrace)
	bookListHandler := kit_grpc.NewServer(makeGetBookListEndpoint(), decodeRequest, encodeResponse, serverTrace)
	// 增加链路监测结束
